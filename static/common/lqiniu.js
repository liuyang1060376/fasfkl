		...
		var params = {
			...
			init: {
				'FileUploaded': function(up,file,info) {
					if(args['success']){
						var success = args['success'];  									//成功时候返回
						file.name = domain + file.target_name;
						success(up,file,info);
					}
				},
				'Error': function(up,err,errTip) {											//失败时候返回
					if(args['error']){
